...
# setwd("C:/Users/admin/Desktop/SelectedData/SoftwareDevelop/")
...
# clinical_filter_patient <- read.table("./clinical_info.txt", sep = "\t", header = T)
# colnames(clinical_filter_patient) <- c("PATIENT_ID", "OS_STATUS", "OS_MONTHS")
# rna_filter_nor_info <- read.table("./rna_info.txt", header = T, row.names = 1, sep = "\t", check.names = F)
# chosen_genes <- readLines("./chosen_genes.txt")
# 
# seed_num = 12
# ratio = 0.5
...
######## Write necessary results into the folder ##############
print("Results are writing to the files......")
writeLines(train_patients, "./train_group_patients.txt")
writeLines(test_patients, "./test_group_patients.txt")
write.table(uni_res$uni_res, file = "./uni_candidate.txt", 
            sep = "\t", row.names = T, col.names = T, quote = F)
write.table(model_details$cox_res, file = "./multi_candidate.txt",
            sep = "\t",row.names = T, col.names = T, quote = F)

uni_cox <- uni_res$uni_res[uni_genes,]
rmarkdown::render("../test.rmd", params = list(my_formula = model_details$formula,
                                              uni_cox = uni_cox,
                                              multi_cox = model_details$cox_res,
                                              train_thresh = round(train_surv$threshold, 3),
                                              train_survival_plot = train_surv$surv_plot,
                                              test_thresh = round(test_surv$threshold, 3),
                                              test_survival_plot = test_surv$surv_plot,
                                              total_thresh = round(total_surv$threshold, 3),
                                              total_survival_plot = total_surv$surv_plot
                                              )
)
